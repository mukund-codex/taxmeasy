<?php
	...
	$sql = "SELECT cp.*, cl.name, cl.email, cl.mobile, cl.username FROM ".PREFIX.$tableName." cp JOIN clients cl ON cl.id = cp.client_id WHERE cp.deleted_at IS NULL order by cp.created_at DESC";
